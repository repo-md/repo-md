...
const output = {
  ...
  summary: {
    ...
    functionsWithDefaults: functions.filter(f => 
      Object.values(f.parameters?.properties || {}).some(p => p.default !== undefined)
    ).map(f => f.name),
    
    // Functions with required parameters
    functionsWithRequired: functions.filter(f => 
      f.parameters?.required?.length > 0
    ).map(f => ({
      name: f.name,
    ...
    coverage: {
      ...
      missingSchemasInSpecs: Object.keys(schemas).filter(name => 
        !functions.some(f => f.name === name)
      ),
      extraSpecsNotInSchemas: functions.filter(f => 
        !Object.keys(schemas).includes(f.name)
      ).map(f => f.name)
    }
  }
};
...
console.log(`   Coverage: ${output.summary.coverage.allSchemasCovered ? '✅ Complete' : '❌ Incomplete'}`);

if (output.summary.coverage.missingSchemasInSpecs.length > 0) {
  console.log(`   Missing specs: ${output.summary.coverage.missingSchemasInSpecs.join(', ')}`);
}

if (output.summary.coverage.extraSpecsNotInSchemas.length > 0) {
  console.log(`   Extra specs: ${output.summary.coverage.extraSpecsNotInSchemas.join(', ')}`);
}

console.log(`   Functions with defaults: ${output.summary.functionsWithDefaults.length}`);
console.log(`   Functions with required params: ${output.summary.functionsWithRequired.length}`);
